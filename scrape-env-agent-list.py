...
def getListUrl(page):
    ...
    listLoc = parsedHtmls.xpath(
        '//table[@id = "contentTable"]/tbody/tr/td[3]/a/text()')
    listLoc = [re.sub(r'(\s)', '', u) for u in listLoc]

    list近三年编制报告书数量 = parsedHtmls.xpath(
        '//table[@id = "contentTable"]/tbody/tr/td[6]/text()')
    list近三年编制报告书数量 = [re.sub(r'(\s)', '', u) for u in list近三年编制报告书数量]

    list近三年编制报告表数量 = parsedHtmls.xpath(
        '//table[@id = "contentTable"]/tbody/tr/td[7]/text()')
    list近三年编制报告表数量 = [re.sub(r'(\s)', '', u) for u in list近三年编制报告表数量]
    ...
    listCode = listLocAndCode[5::12]
    listStatus = listLocAndCode[9::12]
    listInfo = [[a, b, c, d, e, f, g]
                for (a, b, c, d, e, f, g) in zip(listName, listUrl, listCode, listLoc, list近三年编制报告书数量, list近三年编制报告表数量, listStatus)]
    return listInfo


def getUnitInfo(unitInfo):
    unitUrl = f"http://114.251.10.92:8080/XYPT/score/detailInfo?objectType=2&objectId={unitInfo[1]}"
